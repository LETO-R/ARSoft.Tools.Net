...
namespace ARSoft.Tools.Net.Dns
{
	...
	/// </summary>
	public class DnsSocksClient
	{
		private const int _DNS_PORT = 53;
		private readonly List<IPAddress> _dnsServers;
	    private readonly IPAddress _socksIP;
	    private readonly short _socksPort;
		
		/// <summary>
		/// Max amount of time allowed while performing a query.
		/// </summary>
		public int QueryTimeout { get; set; }
		
	    /// <summary>
	    /// Provides a new instance that'll try to use the system-configured DNS servers with a timeout of 10 seconds. Will connect through the specified SOCKS4 server.
	    /// </summary>
	    /// <param name="socksIP">SOCKS4 server IP address.</param>
	    /// <param name="socksPort">SOCKS4 server port.</param>
        public DnsSocksClient(IPAddress socksIP, short socksPort)
            : this(socksIP, socksPort, GetDnsServers(), 10000)
		{

		}

	    /// <summary>
	    /// Provides a new instance with custom dns server and query timeout. Will connect through the specified SOCKS4 server.
	    /// </summary>
	    /// <param name="socksIP">SOCKS4 server IP address.</param>
	    /// <param name="socksPort">SOCKS4 server port.</param>
        ...
        public DnsSocksClient(IPAddress socksIP, short socksPort, IPAddress dnsServers, int queryTimeout)
	    {
	        _socksIP = socksIP;
	    ...
	    /// <summary>
	    /// Provides a new instance with custom dns servers and query timeout. Will connect through the specified SOCKS4 server.
	    /// </summary>
	    /// <param name="socksIP">SOCKS4 server IP address.</param>
	    /// <param name="socksPort">SOCKS4 server port.</param>
	    /// <param name="dnsServers">The IPAddresses of the dns servers to use</param>
	    /// <param name="queryTimeout">Query timeout in milliseconds</param>
        public DnsSocksClient(IPAddress socksIP, short socksPort, List<IPAddress> dnsServers, int queryTimeout)
        {
            _socksIP = socksIP;
            _socksPort = socksPort;
			...
			where TMessage: DnsMessageBase, new()
		{
			...
			for (int i = 0; i < _dnsServers.Count; i++)
			{
			    ...
			    byte[] resultData = QueryByTcp(nameServer, messageData, messageLength); // Always TCP (so we can transfer over the proxy)
		...
		private byte[] QueryByTcp(IPAddress nameServer, byte[] messageData, int messageLength)
		{
            try
            {
                using (TcpClient tcpClient = Socks.Socks.ConnectTcpClientTo(_socksIP, _socksPort, nameServer, _DNS_PORT, "DNS_CLIENT"))
                {
                    try
                    {
                        byte[] resultData;
            ...
            {
                Trace.TraceError("Error on dns query: " + ex);
                return null;
            }
		}
		...
		private static List<IPAddress> GetDnsServers()
		{
			...
			if (res.Count == 0)
			{
				res.Add(Fallbacks.PrimaryDNS);
				res.Add(Fallbacks.SecondaryDNS);
			}
